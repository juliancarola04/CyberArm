...
3. Cloná el repositorio. Si tenés Git instalado hacé simplemente ```git clone https://github.com/juliancarola04/CyberArm/```. Si NO tenés Git bajate los archivos manualmente.

![Git archivos manuales](assets/img/GitArchivosManual.png "Git archivos manuales")
...
4. Dale click al ícono de la extensión que se formó a la derecha y abrí la carpeta de ESP32.

![Abrir proyecto PlatformIO](assets/img/AbrirProyectoPlatformIO.png "Abrir proyecto PlatformIO")

5. En la carpeta "src" se va a encontrar el archivo ```credenciales.h```. Acá colocá el nombre de la red WiFi en ```ssid``` y la contraseña en ```password```

6. En ```main.cpp``` colocá los pines de los servos en la función setup y colocá los valores por defectos de los ángulos de los servos

7. Abrí el Administrador de dispositivos de Windows.

8. Una vez tengas cargado eso, andá a ```platformio.ini``` y colocá el puerto del USB donde tengas enchufado el ESP32.

![Puerto ESP32](assets/img/PuertoESP32.png "Puerto ESP32")

9. Cargá el código al ESP32 y si querés abrí la consola serial para monitorear lo que le llegue por puerto serial.
